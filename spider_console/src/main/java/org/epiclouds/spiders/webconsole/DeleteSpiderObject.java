...
public class DeleteSpiderObject extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * Default constructor.
	 */
	public DeleteSpiderObject() {
	...
	protected void doGet(HttpServletRequest request,
		...
		ConsoleCommandBean result=ConsoleCommandManager.getManager().handleCommand(
				new ConsoleCommandBean(AbstractConsoleCommandHandler.ConsoleCommand.DELETE,
				SpiderStatusManager.getSpiderClassBean(classname)),objectname);
		if(result!=null&&result.getCommand()==AbstractConsoleCommandHandler.ConsoleCommand.DELETESUCCESS){
			request.setAttribute("success", "删除爬虫对象"+objectname+"成功！");
			request.getRequestDispatcher("success.jsp").forward(request, response);
			return;
		}
		
		if(result==null||result.getCommand()!=AbstractConsoleCommandHandler.ConsoleCommand.DELETESUCCESS){
			request.setAttribute("error", "删除爬虫对象出错！"+result);
			request.getRequestDispatcher("error.jsp").forward(request, response);
			return;
		}
