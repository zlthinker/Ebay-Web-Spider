...
public class StopSpiderObject extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * Default constructor.
	 */
	public StopSpiderObject() {
	...
	protected void doGet(HttpServletRequest request,
		...
		ConsoleCommandBean result=ConsoleCommandManager.getManager().handleCommand(
				new ConsoleCommandBean(AbstractConsoleCommandHandler.ConsoleCommand.STOP,
				SpiderStatusManager.getSpiderClassBean(classname)),objectname);
		if(result!=null&&result.getCommand()==AbstractConsoleCommandHandler.ConsoleCommand.STOPSUCCESS){
			request.setAttribute("success", "停止爬虫对象"+objectname+"成功！");
			request.getRequestDispatcher("success.jsp").forward(request, response);
			return;
		}
		
		if(result==null||result.getCommand()!=AbstractConsoleCommandHandler.ConsoleCommand.STOPSUCCESS){
			request.setAttribute("error", "停止爬虫对象出错！"+result);
			request.getRequestDispatcher("error.jsp").forward(request, response);
			return;
		}
